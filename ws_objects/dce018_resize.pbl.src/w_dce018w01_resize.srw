$PBExportHeader$w_dce018w01_resize.srw
forward
global type w_dce018w01_resize from window
end type
type cb_retrieve from commandbutton within w_dce018w01_resize
end type
type cb_insert from commandbutton within w_dce018w01_resize
end type
type cb_delete from commandbutton within w_dce018w01_resize
end type
type dw_1 from datawindow within w_dce018w01_resize
end type
type cb_clear from commandbutton within w_dce018w01_resize
end type
type cb_save from commandbutton within w_dce018w01_resize
end type
type cb_close from commandbutton within w_dce018w01_resize
end type
type mle_output from multilineedit within w_dce018w01_resize
end type
type mle_view from multilineedit within w_dce018w01_resize
end type
type lb_items from listbox within w_dce018w01_resize
end type
type gb_3 from groupbox within w_dce018w01_resize
end type
type gb_4 from groupbox within w_dce018w01_resize
end type
type gb_5 from groupbox within w_dce018w01_resize
end type
type mle_describe from multilineedit within w_dce018w01_resize
end type
type gb_1 from groupbox within w_dce018w01_resize
end type
type gb_2 from groupbox within w_dce018w01_resize
end type
end forward
...
string title = "DataWindow Resize Event"
...
global w_dce018w01_resize w_dce018w01_resize
...
mle_describe.text += "2.Testing Purpose : DataWindow Resize Event " // This must include
...
lb_items.additem("DCE018W01P001: Change DataWindow Width Property")
lb_items.additem("DCE018W01P002: Change DataWindow Height Property")
lb_items.additem("DCE018W01P003: Change DataWindow Size Using Function Resize")
lb_items.additem("DCE018W01P004: Using Mouse to ReSize the Window")
end subroutine
...
Choose Case ls_item
	Case 'DCE018W01P001'
		if ab_execute then
			//Change DataWindow Width Property
			Wf_output('Test Point: ' + ls_item,true)
			Wf_output('Describe  : ' + ls_descript,False)
			dw_1.Width = dw_1.Width - 20
		else
			
		end if
	Case 'DCE018W01P002'
		if ab_execute then
			//Change DataWindow Height Property
			Wf_output('Test Point: ' + ls_item,true)
			Wf_output('Describe  : ' + ls_descript,False)
			dw_1.Height = dw_1.Height - 20
		else
			
		end if
	Case 'DCE018W01P003'
		if ab_execute then
			//Change DataWindow Size Using Function Resize
			Wf_output('Test Point: ' + ls_item,true)
			Wf_output('Describe  : ' + ls_descript,False)
			dw_1.Resize(dw_1.Width - 20,dw_1.Height - 20)
		else
			
		end if
	Case 'DCE018W01P004'
		if ab_execute then
			//Change DataWindow Size Using Trigger Event 
		else
			Wf_output("Please using mouse to change the window's size",true)
		end if
	Case Else
...
event open;
declare my_proc procedure for dec_emp_init using sqlca;
execute my_proc;
commit  using sqlca;
...
event resize;//add by guojun at 2003-12-17
dw_1.resize(newwidth - dw_1.x - 100,dw_1.height)
gb_2.resize(newwidth - gb_2.x - 50,gb_2.height)
end event
...
integer taborder = 20
string title = "none"
string dataobject = "d_dce018w01_column"
boolean minbox = true
boolean maxbox = true
...
event resize;string ls_output

ls_output=string(mle_output.LineCount(),'00')+' Information: '+" Resize "+" (SizeType="+string(sizetype)+", NewWidth="+string(newwidth)+",NewHeight = "+string(newheight)+")"
